...
def ctc_decoder(predictions: np.ndarray, chars: typing.Union[str, list]) -> typing.List[str]:
    ...
    # use group by to find continuous same indexes
    grouped_predictions = [[k for k, _ in groupby(preds)] for preds in argmax_predictions]

    # convert indexes to chars
    texts = ["".join([chars[k] for k in group if k < len(chars)]) for group in grouped_predictions]
...
def edit_distance(prediction_tokens: typing.List[str], reference_tokens: typing.List[str]) -> int:
    ...
    for i, p_tok in enumerate(prediction_tokens):
        for j, r_tok in enumerate(reference_tokens):
            ...
            if p_tok == r_tok:
                dp[i+1][j+1] = dp[i][j]
            # If the tokens are different, the edit distance is the minimum of the previous entries
            else:
                dp[i+1][j+1] = min(dp[i][j+1], dp[i+1][j], dp[i][j]) + 1

    # Returns the final entry in the matrix as the edit distance
    return dp[-1][-1]
...
def get_cer(
    ...
    Args:
        ...
        targets (typing.Union[str, typing.List[str]]): list of target words
    ...
    total, errors = 0, 0
    for pred_tokens, target_tokens in zip(preds, targets):
...
def get_wer(
    ...
    Args:
        targets (typing.Union[str, typing.List[str]]): string of target sentence or list of target words
        preds (typing.Union[str, typing.List[str]]): string of predicted sentence or list of predicted words

    Returns:
        Word error rate score
    """
    if isinstance(preds, str) and isinstance(targets, str):
        preds = [preds]
        targets = [targets]

    if isinstance(preds, list) and isinstance(targets, list):
        errors, total_words = 0, 0
        for _pred, _target in zip(preds, targets):
        ...
        print("Error: preds and targets must be either both strings or both lists of strings")
        return np.inf
